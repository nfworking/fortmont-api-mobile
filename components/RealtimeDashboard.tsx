...
export function RealtimeDashboard() {
  ...
  useEffect(() => {
    ...
    const loadRealtimeData = async (isBackgroundRefresh: boolean) => {
      if (requestInFlight) {
        return;
      }
    ...
    <View className="flex-1">
      <View className="mb-4 overflow-hidden rounded-3xl border border-emerald-900/30 bg-zinc-950 px-5 py-5">
        ...
        <Text className="mt-3 max-w-2xl text-sm leading-5 text-zinc-400">
          Welcome back 🎉 Here is what you currently have running in your homelab
        </Text>

        <View className="mt-4 flex-row flex-wrap items-center gap-3">
          <Pressable
            onPress={() => setRefreshNonce((value) => value + 1)}
            className="flex-row items-center rounded-full border border-zinc-800 bg-zinc-900 px-4 py-2.5 active:opacity-80"
          >
          ...
          <View className="rounded-full border border-zinc-800 bg-zinc-900 px-4 py-2.5">
